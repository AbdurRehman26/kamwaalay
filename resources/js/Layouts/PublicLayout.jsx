            ...
            <nav className="bg-white shadow-md sticky top-0 z-50 border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-6 lg:px-8">
                    <div className="flex justify-between items-center h-20 w-full">
                        <Link to={route('home')} className="text-3xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                            kamwaalay
                        </Link>

                        <div className="hidden lg:flex items-center space-x-8">
                            <Link to={route('home')} className="text-gray-700 hover:text-purple-600 font-medium transition-colors">Home</Link>
                            {user?.role !== 'helper' && (
                                <Link to={route('helpers.index')} className="text-gray-700 hover:text-purple-600 font-medium transition-colors">Find Help</Link>
                            )}
                            {(!user || user.role === 'helper') && (
                                <Link to={route('service-requests.browse')} className="text-gray-700 hover:text-purple-600 font-medium transition-colors">Services Required</Link>
                            )}

                            {user ? (
                                <>
                                    {(user.role === 'user' || user.role === 'business') && (
                                        <Link to={route('bookings.create')} className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-2.5 rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all shadow-md hover:shadow-lg font-medium">
                                            Post Service Request
                                        </Link>
                                    )}
                                    {user.role === 'admin' && (
                                        <Link to={route('admin.dashboard')} className="text-gray-700 hover:text-purple-600 font-medium transition-colors">Admin</Link>
                                    )}
                                    {user.role === 'business' && (
                                        <Link to={route('business.dashboard')} className="text-gray-700 hover:text-purple-600 font-medium transition-colors">Business</Link>
                                    )}
                                    <Link to={route('dashboard')} className="text-gray-700 hover:text-purple-600 font-medium transition-colors">Dashboard</Link>
                                    <button
                                        onClick={async () => {
                                            try {
                                                await logout();
                                    ...
                                    <Link to={route('profile.edit')} className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-2.5 rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all shadow-md hover:shadow-lg font-medium">
                                        Profile
                                    </Link>
                                </>
                            ) : (
                                <>
                                    <Link to={route('login')} className="text-gray-700 hover:text-purple-600 font-medium transition-colors">Login</Link>
                                    <Link to={route('register')} className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-2.5 rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all shadow-md hover:shadow-lg font-medium">
                                        Get Started
                                    </Link>
                                </>
                            )}
                    ...
                    {mobileMenuOpen && (
                        <div className="lg:hidden py-4 space-y-2 border-t border-gray-100">
                            ...
                            {user ? (
                                <>
                                    {(user.role === 'user' || user.role === 'business') && (
                                        <Link to={route('bookings.create')} className="block py-3 px-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all font-medium text-center">
                                            Post Service Request
                                        </Link>
                                    )}
                ...
                <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
                        <div className="col-span-1 md:col-span-2">
                            <h3 className="text-2xl font-bold mb-4 bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">kamwaalay</h3>
